    ...
    return (
      <>
        <button
          ...
          style={{ position: "fixed", right: 18, bottom: 22, zIndex: 60, width: 54, height: 54, borderRadius: "50%", background: accent, color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 10px 26px rgba(5,150,105,0.38)" }}
        >
          <Icon icon="bookmark" size={21} />
          {binder.length > 0 && (
            <span
              style={{
                position: "absolute",
        ...
        {sheetOpen && (
          <div onClick={() => setSheetOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.45)", zIndex: 70, display: "flex", alignItems: "flex-end" }}>
            <div
              onClick={(e) => e.stopPropagation()}
              style={{ width: "100%", maxHeight: "80vh", background: "var(--kz-bg)", borderRadius: "20px 20px 0 0", display: "flex", flexDirection: "column", animation: "kz-rise .22s ease-out" }}
            >
              <div style={{ display: "flex", justifyContent: "center", padding: "10px 0 0" }}>
                <div style={{ width: 36, height: 4, borderRadius: 999, background: "var(--kz-border)" }} />
              </div>
              {header}
              {list}
              {footer}
